...
def authmenu():
    ...
    while True: 
        if not running:
            break
        if not found: #to break the loop later
        ...
        for record in UserData:     
            if record[0] == username and record[1] == password:
                ...
                role = record[2]
                while True: #to loop the menu screen without relogging in after every function
                    if role == "admin":
...
7. Edit minimum""")
                        ...
                        if role == "admin" or role == "inventory-checker":
                            stocktake()
                    elif role == "admin" and functchoice == 6 or role == "purchaser" and functchoice == 2:
                        if role == "admin" or role == "purchaser":
                            replenishlist()
